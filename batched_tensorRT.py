...
def parse_arguments(args=None):
    ...
    parser.add_argument('--output_pkl',
                        type=str,
                        help='pkl file where the text output is stored.',
                        default=None)
    parser.add_argument(
        '--output_logits_npy',
        type=str,
    ...
    if input_file is None:
        if 'whisper' in model_name.lower():
            ...
            for curr_text in input_text:
                curr_text = system_prompt_start + curr_text + system_prompt_end
...
def print_output(tokenizer,
                 ...
                 system_prompt_start=None,
                 system_prompt_end=None):

    batch_size, num_beams, _ = output_ids.size()
    if output_csv is None and output_npy is None and output_pkl is None:
        for batch_idx in range(batch_size):
            inputs = output_ids[batch_idx][0][:input_lengths[batch_idx]].tolist(
            )
            input_text = tokenizer.decode(inputs, skip_special_tokens=False)
    ...
    if output_pkl is not None:
        ...
        for batch_idx in range(batch_size):
            ...
            outputs = output_ids[batch_idx][0][
                output_begin:output_end].tolist()
            output_text = tokenizer.decode(outputs)
            batch_out.append((input_text.replace(system_prompt_start, '').replace(system_prompt_end, ''), output_text))
            print(input_text)
            print(output_text)
        temp_file.extend(batch_out)
...
def main(args):
    ...
    with open(args.input_file, 'rb') as pickle_file:
        all_samples = pickle.load(pickle_file)
        num_samples = len(all_samples)

    # Process only this GPU's portion of the data
    while(i < num_samples):
        batch_end = min(i + batch_size, num_samples)
        batch = all_samples[i:batch_end]
        batch_input_ids = parse_input(tokenizer=tokenizer,
                                      ...
                                      model_name=model_name,
                                      model_version=model_version,
                                      system_prompt_start=args.prepend_system_prompt,
                                      system_prompt_end=args.append_system_prompt)
